...
class Lines():
    ...
    def project_on_road(self, image_input):
        ...
        if self.detected_first & self.detected:
            ...
            offset_of_center = round(np.abs(self.offset/3),2) * 100
            
            if offset_of_center < -10:
                img_text2 = "vehicle is: " + str(offset_of_center) + ' (cm) left of center'
                print("R")
            elif offset_of_center > 10:
                img_text2 = "vehicle is: " + str(offset_of_center) + ' (cm) right of center'
                print("L")
            else:
                img_text2 = "vehicle is: " + str(offset_of_center) + ' (cm) away from center'
                print("F")
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        cv2.imshow("Rpi lane detection", lines.project_on_road(image))
